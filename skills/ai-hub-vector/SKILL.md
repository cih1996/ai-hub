...
|------|-----------|---------|--------|
| 全局 | `knowledge` / `memory` | `~/.ai-hub/knowledge/` | 所有会话可读 |
| 团队 | `<团队名>/knowledge` / `<团队名>/memory` | `~/.ai-hub/teams/<团队名>/knowledge/` | 同团队可读写 |
| 会话 | `<团队名>/sessions/<id>/knowledge` / `memory` | `~/.ai-hub/teams/<团队名>/sessions/<id>/knowledge/` | 仅该会话可读写 |
...
- **写入**（传 scope=`<团队名>/knowledge`）：写入团队级（允许，需属于该团队）
- **写入**（传 scope=`knowledge`）：写入全局（允许）
...
curl -X POST http://localhost:$AI_HUB_PORT/api/v1/vector/search_memory \
  -H "Content-Type: application/json" \
  -d '{"query": "搜索内容", "top_k": 5, "session_id": '"$AI_HUB_SESSION_ID"', "tags": ["纠错"]}'
```
...
curl -X POST http://localhost:$AI_HUB_PORT/api/v1/vector/write_knowledge \
  -H "Content-Type: application/json" \
  -d '{"file_name": "项目名-主题.md", "content": "文件内容", "scope": "AI Hub 维护团队/knowledge", "session_id": '"$AI_HUB_SESSION_ID"'}'

# 写入全局
curl -X POST http://localhost:$AI_HUB_PORT/api/v1/vector/write_knowledge \
  -H "Content-Type: application/json" \
  -d '{"file_name": "通用主题.md", "content": "文件内容", "scope": "knowledge"}'
```
...
```json
{
  ...
  "files": [
    ...
    {"file_name":"项目概览.md","preview":"# 项目...","type":"knowledge","source_session_id":25,"updated_at":"2026-03-03T20:00:00+08:00","scope":"AI Hub 维护团队/knowledge","origin":"team"},
    {"file_name":"通用知识.md","preview":"# 通用...","type":"knowledge","source_session_id":0,"updated_at":"2026-03-01T12:00:00+08:00","scope":"knowledge","origin":"global"}
  ]
}
```
...
- 需要写入团队级共享知识时，显式传 `scope=<团队名>/knowledge`
- 需要写入全局通用知识时，显式传 `scope=knowledge`
...
> - 会话 knowledge：`~/.ai-hub/teams/<团队名>/sessions/<id>/knowledge/<file_name>`
